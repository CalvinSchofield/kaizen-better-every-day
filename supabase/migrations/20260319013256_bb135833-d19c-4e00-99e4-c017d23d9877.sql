...
BEGIN
  ...
  IF v_existing.is_finalized = true AND p_is_finalized IS DISTINCT FROM false THEN
    IF p_sales_log IS NOT NULL AND jsonb_array_length(COALESCE(p_sales_log, '[]'::jsonb)) > 0 THEN
      ...
      -- Start with existing sales that are NOT in the incoming set (keep untouched ones)
      -- Then add ALL incoming sales (which may be updates to existing ones or brand new)
      v_merged_sales := '[]'::jsonb;
      
      -- Keep existing sales not present in incoming payload
      ...
      -- Add all incoming sales (new or updated versions of existing ones)
      v_merged_sales := v_merged_sales || p_sales_log;
      
      -- Count funded sales (exclude cancelled/never_installed)
      ...
      -- Recalculate fp_plus/prmr/upgrade_prmr from merged sales_log
      SELECT 
  ...
  IF v_existing.id IS NOT NULL THEN
    ...
    -- When finalizing (p_is_finalized = true), incoming sales take priority over existing ones
    -- This ensures install_status updates from ScheduledInstallStep are persisted
    IF p_is_finalized = true AND p_sales_log IS NOT NULL THEN
      ...
      -- Add all incoming sales (updated versions with install_status)
      v_merged_sales := v_merged_sales || p_sales_log;
    ELSE
      -- Normal (non-finalize) merge: only add truly new sales
  ...
  -- Check if merged sales_log has entries - if so, compute totals from it
