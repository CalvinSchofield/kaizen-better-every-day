...
async function fetchChatResponse(chatUrl: string, messages: Message[], sessionToken: string, stream: boolean) {
...
async function requestNonStreamingChat({
  ...
  let resp: Response;
  try {
    resp = await fetchChatResponse(chatUrl, messages, sessionToken, false);
  } catch {
...
async function streamChat({
  ...
  if (isNativeApp()) {
    await requestNonStreamingChat({ chatUrl, messages, sessionToken, onDelta, onDone, onError });
    return;
  }

  let resp: Response;
  try {
    resp = await fetchChatResponse(chatUrl, messages, sessionToken, true);
  } catch {
    onError("Network error. Please check your connection.");
    return;
  }
...
export const ChatOverlay = ({
  ...
  return (
    ...
    >
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-6 py-8">
            <div className="text-center space-y-2">
              ...
              <h2 className="text-xl font-bold text-foreground">{emptyStateTitle}</h2>
      ...
      {/* Input */}
      <div className="border-t border-border/50 bg-background px-4 py-3 pb-3">
