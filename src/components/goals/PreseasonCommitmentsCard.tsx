...
const formatTrainingTime = (minutesPerWeek: number | null | undefined): string => {
  if (!minutesPerWeek) return "0m/week";
  const hours = Math.floor(minutesPerWeek / 60);
  const mins = minutesPerWeek % 60;
  if (hours > 0 && mins > 0) return `${hours}hr ${mins}m/week`;
  if (hours > 0) return `${hours}hr/week`;
  return `${mins}m/week`;
};

export const PreseasonCommitmentsCard = ({ goals, onIncrementRolePlays, defaultOpen = false }: PreseasonCommitmentsCardProps) => {
  ...
  // Build a compact preview for the collapsed state
  const previewParts: string[] = [];
  if (totalBooks > 0) previewParts.push(`${totalRead}/${totalBooks} books`);
  if (rolePlaysGoal > 0) previewParts.push(`${rolePlaysDone}/${rolePlaysGoal} role plays`);
  if (goals.training_hours_goal) previewParts.push(formatTrainingTime(goals.training_hours_goal));
  const previewText = previewParts.join(" · ") || "No commitments set";
          ...
          <div className="px-4 pb-4 space-y-3">
            ...
            <div className="rounded-lg bg-muted/40 p-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium">Weekly Training</p>
                  <p className="text-xs text-muted-foreground">
                    Product, door approach, in-home process, competitors
                  </p>
                </div>
