...
import { Clock, CheckCircle2, XCircle, Loader2, Check, ChevronsUpDown, ChevronLeft, User, MapPin, Sparkles, Users, AlertTriangle, Heart } from "lucide-react";
...
const RELATIONSHIPS = [
  'Friend', 'Roommate', 'Coworker', 'Family', 'Classmate', 'Acquaintance', 'Other',
];
...
const LocationCombobox = ({ value, onValueChange, options, hasError }: { value: string; onValueChange: (v: string) => void; options: string[]; hasError: boolean }) => {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className={cn("w-full justify-between font-normal h-12 text-base", !value && "text-muted-foreground", hasError && "border-destructive ring-destructive")}
        >
          {value || "Select state..."}
      ...
      <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
        <Command>
          ...
          <CommandList>
            ...
            <CommandGroup>
              {options.map((loc) => (
                <CommandItem key={loc} value={loc} onSelect={() => { onValueChange(loc); setOpen(false); hapticLight(); }}>
...
export default function AddRecruit() {
  ...
  const isMgmtOrAbove = teamAccess?.accessLevel === 'mgmt_group_lead' || teamAccess?.accessLevel === 'area_director' || accessLevel === 'corporate';
  ...
  const { data: notionOptions, isLoading: optionsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data as { locationOptions: string[]; recruitmentSourceOptions: string[] };
    },
  ...
  const { data: currentRep } = useQuery<CurrentRepIdentity | null>({
    ...
    queryFn: async () => {
      ...
      if (user.email) {
        const { data: byEmail } = await supabase.from('reps').select('id, name, team_leader, user_id, email').ilike('email', user.email).maybeSingle();
  ...
  const { data: teamLeaderData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from('reps').select('id, user_id, name').ilike('name', `%${searchName}%`).limit(1).maybeSingle();
      return data;
    },
  ...
  const allRecruiters = useMemo(() => {
    ...
    const requiredIds = [currentRep?.id, selectedRecruiter].filter(Boolean) as string[];
  ...
  // Pre-fill defaults on mount
  useEffect(() => {
    if (hasInitializedRef.current) return;
    if (!isLeader && isLeader !== false) return; // wait for teamAccess
  ...
  }, [isLeader, currentRep, allRecruiters]);
  ...
  const createRecruitMutation = useMutation({
    ...
    onError: (error) => {
      ...
      toast.error('Failed to add recruit', { description: error instanceof Error ? error.message : 'Please try again' });
    },
  });
  ...
  // Navigation
  const goNext = () => {
    if (step < totalSteps - 1) { setDirection(1); setStep(s => s + 1); hapticLight(); }
  };
  const goBack = () => {
    if (step > 0) { setDirection(-1); setStep(s => s - 1); hapticLight(); }
    else navigate(-1);
  };
  ...
  const handleLeaderSubmit = async () => {
    ...
    if (!emailValid) { toast.error('Valid email required for Signed stage'); return; }

    // Resolve the recruiter's auth user_id to pass to the edge function
    const recruiterData = allRecruiters.find(r => r.id === selectedRecruiter);
  ...
  const handleRepSubmit = async () => {
    if (!teamLeaderData?.user_id) { toast.error('Could not find your team leader'); return; }
    try {
      await submitMutation.mutateAsync({
        name: name.trim(), phone: phone.trim(), relationship, notes,
        teamLeaderUserId: teamLeaderData.user_id,
        suggestedByName: currentRep?.name || 'Unknown',
      });
      hapticSuccess();
      toast.success('Suggestion submitted!', { description: `${getCleanName(currentRep?.team_leader)} will review ${name}` });
      navigate('/mygroup');
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'approved': return <div className="flex flex-col items-end gap-0.5"><Badge variant="default" className="bg-green-500"><CheckCircle2 className="h-3 w-3 mr-1" /> They're doing it!</Badge><span className="text-[10px] text-muted-foreground">Help them prepare!</span></div>;
      case 'rejected': return <Badge variant="secondary"><XCircle className="h-3 w-3 mr-1" /> Not interested for now</Badge>;
      default: return null;
    }
  };
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <Label>Name</Label>
                <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Their full name" className="h-12 text-base mt-1" autoFocus />
              </div>
              <div>
                <Label>Phone</Label>
                <Input value={phone} onChange={handlePhoneChange} placeholder="(555) 123-4567" type="tel" className="h-12 text-base mt-1" />
              </div>
              <div>
                <Label>Email {isSignedStage && <span className="text-destructive">*</span>}</Label>
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="their.email@example.com" className="h-12 text-base mt-1" />
                <p className="text-xs text-muted-foreground mt-1">Required when moving to Signed</p>
              </div>
            </div>
            <Button className="w-full h-12" onClick={goNext} disabled={!isStep1Valid}>Continue</Button>
          </div>
        );
      case 1:
        return (
          <div className="space-y-6">
            ...
            <LocationCombobox value={location} onValueChange={setLocation} options={locationOptions} hasError={false} />
            <Button className="w-full h-12" onClick={goNext} disabled={!isStep2Valid}>Continue</Button>
          </div>
        );
      case 2:
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {allSourceOptions.slice(0, 8).map((source) => (
                <button key={source.id} type="button" onClick={() => { setRecruitmentSource(source.id); hapticLight(); }}
                  className={cn("flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all",
                    recruitmentSource === source.id ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"
                  )}
                >
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-1 p-1 bg-muted rounded-lg">
                {RECRUIT_STAGES.map((stage) => (
                  <button key={stage} type="button" onClick={() => { setSelectedStage(stage); hapticLight(); }}
                    className={cn("flex-1 py-2 px-2 text-xs font-medium rounded-md transition-all",
                      selectedStage === stage ? "bg-background shadow-sm" : "text-muted-foreground hover:text-foreground"
                    )}
                  >{stage}</button>
                ))}
              </div>
            </div>
            <Button className="w-full h-12" onClick={goNext} disabled={!isStep3Valid}>Continue</Button>
          </div>
        );
      case 3:
        return (
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select value={selectedRecruiter} onValueChange={handleRecruiterChange}>
                  <SelectTrigger className="h-12 mt-1"><SelectValue placeholder="Select recruiter" /></SelectTrigger>
                  <SelectContent>
                    {filteredRecruiters.map((recruiter) => (
                      <SelectItem key={recruiter.id} value={recruiter.id}>
                        <div className="flex flex-col items-start">
                          <span>{getCleanName(recruiter.name)} {recruiter.id === currentRep?.id ? '(You)' : ''}</span>
                          {recruiter.teamName && <span className="text-xs text-muted-foreground">{getCleanName(recruiter.teamName)}</span>}
                        </div>
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div>
                <Label className="flex items-center gap-1.5"><AlertTriangle className="h-3.5 w-3.5 text-amber-500" /> Things to Look Out For</Label>
                <Textarea value={cautionNotes} onChange={(e) => setCautionNotes(e.target.value)} placeholder="Any concerns or notes for the team..." className="mt-1" rows={2} />
              </div>
            </div>
            <Button className="w-full h-12" onClick={goNext} disabled={!isStep4Valid}>Review</Button>
          </div>
        );
      case 4:
        return (
          <div className="space-y-6">
            ...
            <div className="bg-muted/50 rounded-xl p-4 space-y-3">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">Source</span><span className="font-medium">{recruitmentSource}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Stage</span><Badge variant="secondary">{selectedStage}</Badge></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Recruiter</span><span className="font-medium">{recruiterName}</span></div>
              {teamName && <div className="flex justify-between"><span className="text-muted-foreground">Team</span><span className="font-medium">{teamName}</span></div>}
              {spouseName && <div className="flex justify-between"><span className="text-muted-foreground">Spouse</span><span className="font-medium">{spouseName}</span></div>}
              {cautionNotes && (
                <div className="pt-2 border-t">
                  <div className="flex items-center gap-1.5 text-amber-600 mb-1"><AlertTriangle className="h-3.5 w-3.5" /><span className="text-xs font-medium">Watch Out</span></div>
                  <p className="text-sm text-muted-foreground">{cautionNotes}</p>
                </div>
              )}
            </div>
            <Button className="w-full h-12" onClick={handleLeaderSubmit} disabled={createRecruitMutation.isPending}>
              {createRecruitMutation.isPending ? (<><Loader2 className="h-4 w-4 animate-spin mr-2" />Adding...</>) : 'Add to Pipeline'}
            </Button>
          </div>
        );
      default: return null;
          ...
          {mySuggestions?.length === 0 ? (
            ...
            mySuggestions?.map((suggestion) => (
              <div key={suggestion.id} className="bg-muted/50 rounded-lg p-3">
                ...
                <p className="text-xs text-muted-foreground mt-1">Submitted {format(parseISO(suggestion.created_at), 'MMM d, yyyy')}</p>
              </div>
            ))
          )}
        </div>
    ...
    switch (step) {
      case 0:
        return (
          <div className="space-y-6">
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-3"><User className="h-6 w-6 text-primary" /></div>
            ...
            <div className="space-y-4">
              <div><Label>Name</Label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Their full name" className="h-12 text-base mt-1" autoFocus /></div>
              <div><Label>Phone</Label><Input value={phone} onChange={handlePhoneChange} placeholder="(555) 123-4567" type="tel" className="h-12 text-base mt-1" /></div>
            </div>
            <Button className="w-full h-12" onClick={goNext} disabled={!isStep1Valid}>Continue</Button>
          </div>
        );
      case 1:
        return (
          <div className="space-y-6">
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-3"><Users className="h-6 w-6 text-primary" /></div>
              <h3 className="text-lg font-semibold">How do you know {name.split(' ')[0] || 'them'}?</h3>
            </div>
            <div className="grid grid-cols-2 gap-2">
              {RELATIONSHIPS.map((rel) => (
                <button key={rel} type="button" onClick={() => { setRelationship(rel); hapticLight(); }}
                  className={cn("p-3 rounded-xl border-2 transition-all text-center", relationship === rel ? "border-primary bg-primary/5" : "border-border hover:border-primary/50")}
                ><span className="text-sm font-medium">{rel}</span></button>
              ))}
            </div>
            <div><Label>Notes (optional)</Label><Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Any helpful context..." className="mt-1" rows={2} /></div>
            <Button className="w-full h-12" onClick={goNext}>Review</Button>
          </div>
        );
      case 2:
        return (
          <div className="space-y-6">
            <div className="text-center">
              <div className="w-12 h-12 rounded-full bg-green-500/10 flex items-center justify-center mx-auto mb-3"><CheckCircle2 className="h-6 w-6 text-green-500" /></div>
              <h3 className="text-lg font-semibold">Review & Submit</h3>
            </div>
            <div className="bg-muted/50 rounded-xl p-4 space-y-3">
              <div className="flex justify-between"><span className="text-muted-foreground">Name</span><span className="font-medium">{name}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Phone</span><span className="font-medium">{phone}</span></div>
              {relationship && <div className="flex justify-between"><span className="text-muted-foreground">Relationship</span><span className="font-medium">{relationship}</span></div>}
              {notes && <div className="pt-2 border-t"><span className="text-xs text-muted-foreground">Notes</span><p className="text-sm mt-1">{notes}</p></div>}
            </div>
            <div className="bg-muted/50 rounded-lg p-3 text-sm text-muted-foreground">
              <p>Your suggestion will be sent to <strong>{currentRep?.team_leader || 'your team leader'}</strong> for review.</p>
            </div>
            <Button className="w-full h-12" onClick={handleRepSubmit} disabled={submitMutation.isPending}>
              {submitMutation.isPending ? (<><Loader2 className="h-4 w-4 animate-spin mr-2" />Submitting...</>) : 'Submit Suggestion'}
            </Button>
          </div>
        );
      default: return null;
    ...
    <div>
      ...
      <div className="px-4 py-6 max-w-md mx-auto">
        ...
        {optionsLoading && step === 2 && (
          <div className="flex items-center justify-center py-4 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin mr-2" />Loading options...</div>
        )}
