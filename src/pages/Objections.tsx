...
import { ArrowLeft, Search, X, ChevronDown, Star, MessageCircle } from "lucide-react";
...
const objections: Objection[] = [
  {
    ...
    responses: [
      "That's exactly why I'm here! How long ago did you get it? Awesome. Your neighbors that had the ____ system and loved it as well, the reason why they jumped on board is because of ______, check this out.",
  ...
  },
  {
    ...
    responses: [
      "Of course it is — it's so nice here. And all of your neighbors are so nice! 😄 That's actually exactly why they send me out here. Nobody is getting the system because they worry someone is going to break in tonight. Everyone is loving _______, check this out.",
  ...
  },
  {
    ...
    responses: [
      "100%, that makes total sense — we just met 20 mins ago. I'm the same way by the way, I know where all my money goes and am super sure before I make a decision. But ______ was super important to you and the price is affordable like you said. Let's fast forward. What would you google and look up?",
  ...
  },
  {
    ...
    responses: [
      "Makes total sense. Like you said before we started though, you were the one that could handle this and make the call! You handle the budget and know they wouldn't be mad at you for setting up the system. With that being said, if I weren't here and they were, what would you talk to them about?",
  ...
  },
  {
    ...
    responses: [
      "All good, here's the 12 second pitch… (continue with a shorter pitch and focus on the deal. They're probably not that busy)",
  ...
  },
  {
    ...
    responses: [
      "Of course nobody wants to be locked into something forever. There isn't a service contract so there is some flexibility. But the most important thing for you to know is that our average customer is with us for 10 years. That says a lot about the quality of the product and the customer service.",
  ...
  },
  {
    ...
    responses: [
      "That's totally fine. This may not be for you. Just so you know,…",
  ...
  },
  {
    ...
    responses: [
      "Yes totally can get you that information. Just so you know…",
  ...
  },
  {
    ...
    responses: [
      "Hey trust me I get it. I'm from Dallas — we all have guns and dogs lol. The reason why I'm here…",
  ...
  },
  {
    ...
    responses: [
      "Like a million bucks… Just kidding that's exactly why I'm here. Check this out.",
    ],
  },
];
...
const Objections = () => {
  ...
  const filteredObjections = useMemo(() => {
    ...
    return objections.filter(
      (obj) =>
        ...
        obj.responses.some((r) => r.toLowerCase().includes(query))
    );
  }, [searchQuery]);
  ...
  const toggleFavorite = (objectionId: string, responseIndex: number) => {
    const newFavorites = { ...favorites };
    if (newFavorites[objectionId] === responseIndex) {
      delete newFavorites[objectionId];
    } else {
      newFavorites[objectionId] = responseIndex;
    }
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-3">
        ...
        <p className="text-sm text-muted-foreground text-center px-4 pb-2">
          Tap to expand · Star your favorite response
        </p>

        {/* Objection Cards */}
        <AnimatePresence mode="popLayout">
          {filteredObjections.map((obj, index) => {
            const favoriteIndex = favorites[obj.id];
            const hasFavorite = favoriteIndex !== undefined;
                        ...
                        <div className="px-4 pb-4 space-y-2">
                          {obj.responses.map((response, i) => {
                            ...
                            return (
                              <button
                                key={i}
                                onClick={() => toggleFavorite(obj.id, i)}
                                className={cn(
                                  "w-full text-left rounded-xl p-3 transition-all duration-200 group relative",
                                  isFavorite
                                    ? "bg-gradient-to-br from-primary/15 via-primary/10 to-primary/5 border border-primary/20"
                              ...
                              >
                                <div className="flex items-start gap-2">
                                  ...
                                  <Star
                                    className={cn(
                                      "w-4 h-4 shrink-0 mt-0.5 transition-all",
                                      isFavorite
