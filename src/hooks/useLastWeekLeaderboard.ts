...
const getMondayOfLastWeek = (date: Date): Date => {
  const d = new Date(date);
  const day = d.getDay();
  const diff = d.getDate() - day + (day === 0 ? -6 : 1) - 7; // Go back one more week
  return new Date(d.setDate(diff));
};

const getSaturdayOfLastWeek = (date: Date): Date => {
  const monday = getMondayOfLastWeek(date);
  const saturday = new Date(monday);
  saturday.setDate(monday.getDate() + 5);
  return saturday;
};

export const useLastWeekLeaderboard = (filterByYear?: string) => {
  return useQuery({
    queryKey: ["last-week-leaderboard", filterByYear],
    queryFn: async () => {
      const today = new Date();
      const monday = getMondayOfLastWeek(today);
      const saturday = getSaturdayOfLastWeek(today);
